...
class Functions:
    ...
    @staticmethod
    def is_in(string_:str, list_:list):
        """Checks if a string exists in a list

        Args:
            string_ (str): a string
            list_(list): a list

        Returns:
            True/False: Whether the email is invalid or not
        """
        if any(item in string_ for item in list_):
            return True
        else:
            return False
    @staticmethod
    ...
    def read_files(project_name):
        ...
        Whole_dataset.drop_duplicates(subset=['id', 'creation_time', 'description_update'], inplace=True)
        bug_evolution_db.drop_duplicates(subset=['bug', 'status', 'detail', 'time'], inplace=True)
        # formating update
        try:
        ...
        Whole_dataset['severity_num'] = Whole_dataset['severity'].replace(['blocker', 'blocking', 'critical', 'grave', 
                                                                           'major', 'regression', 'crash', 'normal', 
                                                                           'minor', 'trivial', 'wishlist', 'enhancement', 'task',
                                                                           'S1', 'S2', 'S3', 'S4', '--', None,
                                                                           'urgent', 'high', 'medium', 'unspecified', 'low'],
                                                                          [6,6,5,5,
                                                                           4,4,4,3,
                                                                           2,1,1,0,0,
                                                                           5,4,3,1,3,3,
        ...
        if ('P1' in Whole_dataset.priority.unique()) and (len(Whole_dataset.priority.unique()) in [5,6]):
            ...
            Whole_dataset['priority_num'] = Whole_dataset['priority'].replace(['lowest', 'low', 'medium', 'high', 'highest'],
                                                                              [1,2,3,4,5])
        elif ('unspecified' in Whole_dataset.priority.unique()) and (len(Whole_dataset.priority.unique())==5):
            Whole_dataset['priority_num'] = Whole_dataset['priority'].replace(['low', 'unspecified', 'medium', 'high', 'urgent'],
                                                                              [1,2,3,4,5])
        elif ('VHI' in Whole_dataset.priority.unique()) and (len(Whole_dataset.priority.unique())==5):
            Whole_dataset['priority_num'] = Whole_dataset['priority'].replace(['VLO', 'LO', 'NOR', 'HI', 'VHI'],
                                                                              [1,2,3,4,5])
        else:
        ...
        try:
            ...
            Whole_dataset['assigned_to_detail.email'] = Whole_dataset['assigned_to'].copy()
            Whole_dataset['assigned_to_detail.email'] = Whole_dataset['assigned_to_detail.email'].map(lambda x: x.lower())
        ...
        with open(os.path.join('dat', 'infeasible_dev_emails.txt'), "r") as fp:
            infeasible_dev_emails = json.load(fp)
        
        return [bug_evolution_db, Whole_dataset, list_of_developers, time_to_fix_LDA, 
                SVM_model, feasible_bugs_actual, glove_embeddings_index, infeasible_dev_emails]
    ...
    def model_kp(p, c, T, LogToConsole=False, verbose=False):
        """
        Knapsack model (used for both RABT and DABT with different inputs.)
        """
        p     = np.array(p)
