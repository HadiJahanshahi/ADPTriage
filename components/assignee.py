...
class Assignee:
    ...
    def __init__(self, email, id_, name, LDA_experience, time_limit_orig):
        ...
        self.accuracy            = []
        self.accuracyT           = []
        self.simultaneous_cap    = 1               # capacity of solving bugs simultaneously
...
#        self.time_limit          = time_limit_orig  # it will change (It is for the older version (DABT) with single slot)
    ...
    def calculating_simultaneous_capacity(self, strategy:str, single_slot:bool):
        """
        It finds the capacity of a developer based on the history of the assignments.
        Then, it resets filled_capacity, time_limit, and schedule for the testing phase.
        
        Args:
            strategy (str): [It can be either "max" or "IQR"]
            single_slot(boolean): [whether to have a single slot or not] 
        """
        if single_slot:
            self.simultaneous_cap = 1
        if strategy.lower() == "max":
            ...
            """
            Being a little optimistic in their capacity, we set it to iqr + 0 or 1.
            """
            ...
            IQR                   = Q3 - Q1
            np.random.seed(self.id_)
            self.simultaneous_cap = math.ceil(Q3 + (1.5*IQR)) + np.random.choice([0,1])
        elif strategy.lower() == "iqr":
            """
            ...
            Q1, Q3                = np.quantile(only_working_days, [0.25,0.75])
            IQR                   = Q3 - Q1
            self.simultaneous_cap = math.ceil(Q3 + (1.5*IQR))
        else:
            raise Exception(f"Strategy can be either Max or IQR. You passed {strategy}.")
    ...
    def assign_bug(self, bug:Bug, time_:int, mode_:str):
        ...
        Older and simpler version for the training period.
        Args:
    ...
    def assign_and_solve_bug(self, bug:Bug, time_:int, mode_:str, resolution_:str,
                             which_slot:int=None, what_time:int=None):
        """[Assign a bug to a developer]
        ...
        if (resolution_ == 'sdabt') and (mode_ == "tracking"):
            """
            If we are in testing mode and use S-DABT, we know exactly when to assign a bug.
            """
 ...
 #           self.time_limit      -= solving_time_ # It is for the older version (DABT)
            bug.time_to_be_assigned = time_- bug.open[0] # Passed time since the bug's introduction 
            self.working_time.append(solving_time_)
            if (resolution_ != 'sdabt'):
                """
                In S-DABT, we have a different strategy to find what time to assign a bug.
        ...
        bug.assigned_time.append(time_)
        if which_slot > 0:
            bug.planned_ahead = True
    
    def update_schedule(self, resolution= "actual"):
        """
        1 means an available slot and 0 means an unavailable slot.
